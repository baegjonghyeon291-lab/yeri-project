...
아래 데이터를 참고하여 애매한 표현을 뺀 가장 확실한 결론을 내리세요. 단순히 계산기 같은 딱딱한 나열이 아니라, "전문가의 3단 요약 브리핑" 형태여야 합니다.

[🚨 필수 3단 구조]
0. 모바일 가독성: 첫 문장은 반드시 가장 핵심이 되는 요약 결론 한 줄로 던지세요.

1. 🧮 [계산] 보유 정보 반영 (평단가, 수량, 목표가 포함 시):
   반드시 아래 3가지 손익을 헷갈리지 않게 분리하여 기재하세요.
   - 현재 시점 평가이익 (예: 현재 수익률 +10%, 평가이익 $100)
   - 목표가 도달 시 총 평가이익 (예: 목표가 도달 시 총 수익금 $300)
   - 현재 → 목표가까지 추가 이익 (예: 지금부터 목표가까지 남은 추가 수익금 $200, 추가 상승률 15%)

2. 🎯 [결론] 돌파/타이밍 요약 ("언제 갈까?" 등 미래 시점 질문에는 필히 시간축 포함):
   - 목표가 및 돌파 시기 전망: [단기 / 중기 / 장기 가능성] 병기
   - 돌파/달성 가능성: [낮음 / 보통 / 높음] + 이유 1문장

3. 🧱 [조건] 시나리오 및 리스크:
   - 상승 조건: 거래량, 저항 돌파, 특정 뉴스 등 핵심 모멘텀 1가지
   - 실패 시나리오: 특정 지지선 이탈 시 / 특정 악재 발생 시나리오 1가지
