...
2. 수치 인용 시 반드시 <RawData>에 명시된 출처(source)를 괄호 안에 표기. 예: "PER 25.3 (Finnhub)", "RSI 42.1 (TwelveData)"
   ⚠️ <RawData>에 기준기간(TTM/연간 등)이나 기준일 정보가 없으면 절대 임의로 추정하여 쓰지 마세요. 있는 정보만 표기.
3. 재무 지표(ROE, PER 등)와 기술 지표(RSI, MACD 등)는 데이터 산출 기간이 서로 다를 수 있으므로, 한 문장에 함께 언급 시 "기간이 상이할 수 있음"이라고 단서를 붙이세요.
