...
function buildDataSummary(data) {
    ...
    if (fundamentals) {
        ...
        lines.push(`D/E: ${fmt(fundamentals.debtToEquity)} | 순이익률: ${fmt(fundamentals.netMargin)} | ROE: ${fmt(fundamentals.roe)}`);
        lines.push(`매출성장(YoY): ${fmt(fundamentals.revenueGrowthYoY)} | 베타: ${fmt(fundamentals.beta)}`);
        if (fundamentals.mktCap) lines.push(`시가총액: ${fmtPrice((fundamentals.mktCap / 1e9).toFixed(1), currency)}B`);
        if (fundamentals.nextEarningsDate) lines.push(`📅 다음 실적 발표: ${fundamentals.nextEarningsDate} (예상EPS: ${fmt(fundamentals.nextEarningsEPS)})`);
    }
    ...
    const newsLines = (news || []).slice(0, 7).map(n => `- [${n.publishedAt}] ${n.title} (${n.source})`);
    if (newsLines.length) lines.push(`[최근 뉴스]\n${newsLines.join('\n')}`);
    else lines.push(`[최근 뉴스] ⚠️ 뉴스 데이터를 가져오지 못했습니다. 뉴스 분석 섹션에서 반드시 "뉴스 데이터 없음"을 명시하고, 공개된 최신 이슈를 기반으로 최대한 분석해주세요.`);
    ...
    if (macro) {
        ...
        lines.push(`[거시경제] ⚠️ 거시경제 데이터 없음 — 거시 분석 섹션에 "시장 데이터 없음"을 명시하세요.`);
    }

    // 데이터 밸런스 경고: 펀더멘털 없을 때 AI에게 명시
    if (!fundamentals) {
        lines.push(`\n⚠️ [분석 주의] 펀더멘털 데이터(PER/EPS/ROE 등)를 가져오지 못했습니다. 재무 분석 섹션에서 반드시 "펀더멘털 데이터 없음"을 명시하고, 기술적 지표와 거시경제 데이터에 집중하여 분석하되, 한쪽으로 치우치지 않게 균형있게 분석해주세요.`);
    }
    // 데이터 소스 요약 (AI가 무엇을 기반으로 답하는지 투명하게)
    lines.push(`\n[데이터 소스] 가격:${price?.source || '없음'} | 펀더멘털:${fundamentals?.source || '없음'} | 뉴스:${newsLines.length > 0 ? '있음(' + newsLines.length + '건)' : '없음'} | 거시:${macro?.source || '없음'}`);
...
────────────────────────────────
`;
    return callOpenAI(prompt, useDeep);
}
...
async function analyzeStockCasual(data, useDeep = false, tone = 'normal') {
    ...
    const prompt = `
종합 분석 질문입니다. 짧고 강하게, 결론 먼저 답변하세요.

[기준일: ${today}] [종목: ${data.companyName || data.ticker} (${data.ticker})] ${horizonNote}

${score.summary}

${dataSummary}

━━ 반드시 이 순서로 (각 섹션 3줄 이내) ━━
① ${t.opener}
② 👉 결론 1줄: "${score.suggestedAction}" — ${score.probability}
   *(총점 ${score.total}/40. 이 문구를 반드시 그대로 사용하거나 더 구체화하세요)*
③ **현재 위치**: 현재가/52주 위치/1개월 흐름 (2줄)
④ **상승 포인트** 2~3개 (기술+펀더+업황 중 유효한 것만)
⑤ **리스크** 2~3개
⑥ **행동 계획** (타이밍 트리거 포함, 총점 ${score.total} 기반)
   👉 ${score.suggestedAction === '분할매수' || score.suggestedAction === '매수 준비'
       ? '1차 진입: $xx~xx / 2차: $xx~xx / 손절: $xx'
       : score.suggestedAction === '관망'
       ? '관망 중. 진입 조건: $xx 도달 또는 EMA20 돌파 확인 후'
       : '매수 금지. 지지선 $xx + 거래량 회복 전까지 대기'}
   👉 목표가: $xx (+X%) | 손절선: $xx (-X%)
⑦ 추가 질문 유도 1줄
${t.ending}

[절대 금지] 거시 설명 4줄 이상 | 결론 마지막 배치 | 기업 소개 문단 | 점수와 다른 행동 출력
`;
    return callOpenAI(prompt, useDeep, tone);
}
...
async function analyzePortfolio(portfolioItems, useDeep = false, tone = 'normal') {
    ...
    const portfolioText = portfolioItems.map(p =>
        `- ${p.name} (${p.ticker}) [${p.market}]: ${p.weight}%`
    ).join('\n');

    const prompt = `
사용자가 아래 포트폴리오를 보내왔습니다. 자산배분 관점에서 분석해주세요.
...
1. **포트폴리오 요약**: 전체 구성 한눈에 정리
2. **섹터 비중**: 어떤 섹터에 얼마나 집중되어 있는지
3. **국가 비중**: 한국/미국/기타 비중
4. **성장주/가치주/방어주 성격**: 전체적인 포트폴리오 성격 분류
5. **집중 리스크**: 한 종목/섹터 쏠림 위험
6. **보완 아이디어**: 분산을 위해 추가 고려할 수 있는 섹터/종목 (헬스케어, 소비재, 금융, ETF 등)
7. **투자 성향 추정**: 이 포트폴리오의 투자자 성향 (공격형/균형형/안정형)
8. **한 줄 총평**
...
3. 딱딱한 구분선 금지, **볼드**와 줄바꿈으로 구분
4. 무조건 매수/매도 권유 금지
5. 보완 아이디어는 "~도 생각해볼 수 있어요" 형태로 자연스럽게
6. 마지막에 개별 종목 분석 유도: "원하면 한 종목씩 자세히 볼게요"
7. 마지막 문구: ${t.ending}
`;
    return callOpenAI(prompt, useDeep, tone);
}
...
async function analyzeStockComparison(data1, data2, useDeep = false, tone = 'normal') {
    ...
    const t = toneStrings(tone);
    const prompt = `
다음 두 종목 데이터를 비교 분석해주세요.

[분석 기준일: ${today}]
[비교종목 A]: ${data1.companyName || data1.ticker} (${data1.ticker})
[비교종목 B]: ${data2.companyName || data2.ticker} (${data2.ticker})
...
1. 질문 의도 확인: "${data1.ticker} vs ${data2.ticker} 비교 기준으로 볼게요."
2. 한 줄 비교 결론 (예: "성장성은 A, 안정성은 B가 유리해 보여요")
3. **성장성 비교** (수치 포함):
   - A: 매출성장률 / EPS 성장
   - B: 매출성장률 / EPS 성장
4. **수익성 비교**:
   ...
   - B: 순이익률 / ROE
5. **밸류에이션 비교**:
   - A PER/PSR vs B PER/PSR → 누가 더 저렴한지
6. **기술적 흐름** (RSI, 1개월):
   - A vs B 간단 비교
7. **어떤 투자자에게 맞는지**:
   - 단타 / 장기 / 안정형별 권장

[절대 금지] 한쪽만 길게 분석 금지, 결론 마지막 배치 금지, 장황한 기업 설명 금지
...
module.exports = { analyzeStock, analyzeStockBuyTiming, analyzeStockSellTiming, analyzeStockRisk, analyzeStockEarnings, analyzeStockCasual, analyzeStockOverheat, analyzeStockValuation, analyzeStockComparison, analyzeETF, analyzePortfolio, analyzeRecommendation, analyzeMarket, analyzeSector, classifyQuery, fallbackChat };
