...
*** 티커 추측 절대 금지 (매우 중요):
- 한국어 회사명인데 정확한 티커 매핑을 확실히 모르면 반드시 ticker: null 반환
- 잘 모르는 한국 회사명을 영어 약자나 비슷해 보이는 미국 티커로 추측하지 마세요
- 예: "치타 넷 서플라이 서비스" → ticker: null (모르면 null)
- 예: "테슬라" → ticker: "TSLA" (확실히 아는 경우만 반환)
- 예: "삼성전자" → ticker: null (name만 반환, 티커는 로컬 DB가 처리)
- 사용자가 직접 대문자로 입력한 티커(AAPL, TSLA 등)는 항상 그대로 반환
